...
### 1.1.1 **FAST算法的基本流程**
1. 在图像中选取一个像素点 p，来判断它是不是关键点。IpI_p等于像素点 p的灰度值。
2. 以r为半径画圆，覆盖p点周围的M个像素，通常情狂下，设置 r=3，则 M=16，如下图所示：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/jpeg/25755350/1742438988796-e353c0bf-1c5d-4a0a-a720-4800db55f828.jpeg)

3. 设置一个阈值t，如果在这 16 个像素点中存在 n 个连续像素点的灰度值都高于Ip+tI_p + t，或者低于Ip−tI_p - t，那么像素点 p 就被认为是一个角点。如上图中的虚线所示，n 一般取值为 12。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438988817-51ebeae8-7015-4e22-ac77-ce3d6e2d1f4a.png)

3. 每一个特征点的 16 像素点都属于下列三类中的一种

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/jpeg/25755350/1742438988842-655aee17-984f-462f-9061-acb9385efe2a.jpeg)

4. 根据这些像素点的分类，特征向量 P 也被分为 3 个子集：Pd ，Ps ，Pb，
5. 定义一个新的布尔变量KpK_p，如果 p 是角点就设置为 Ture，如果不是就设置为 False。
...
为所有的候选角点都确定一个打分函数VV  ， VV 的值可这样计算：先分别计算IpI_p与圆上16个点的像素值差值，取绝对值，再将这16个绝对值相加，就得到了VV 的值 V=∑i16∣Ip−Ii∣ V = \sum_{i}^{16}|I_p-I_i|  最后比较毗邻候选角点的 V 值，把V值较小的候选角点pass掉。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438990172-b066ac1c-1f9e-48da-b9ca-c42b131cbea0.png)
...
σs=σ0s \sigma_s=\sigma_0^s σ0\sigma_0是初始尺度，默认为1.2，原图在第0层。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438988878-796a836b-b835-4be7-a4dc-cf5487084b5a.png)

第s层图像的大小： SIZE=(H∗1σs)×(W∗1σs) SIZE = (H*\frac{1}{\sigma_s})\times(W*\frac{1}{\sigma_s}) 
...
计算方法如下: mpq=∑x,yxpyqI(x,y) m_{pq}=\sum_{x,y}x^py^qI(x,y)  质心位置： C=(m10m00,m01m10) C=(\frac{m_{10}}{m_{00}},\frac{m_{01}}{m_{10}})  主方向： θ=arctan(m01,m10) \theta = arctan(m_{01},m_{10}) 
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438989552-3e7bd6d8-50c1-4239-97b7-0a86fed8ed88.png)

图中一条线段的两个端点就是一组点对，其中第二种方法的结果比较好。

3. **构建描述符**：假设x,y是某个点对的两个端点，p(x),p(y)是两点对应的像素值，则有： t(x,y)={1ifp(x)>p(y)0else t(x,y)=\begin{cases}1	&if p(x)>p(y)\\ 0&	else\end{cases}  对每一个点对都进行上述的二进制赋值，形成BRIEF的关键点的描述特征向量，该向量一般为 128-512 位的字符串，其中仅包含 1 和 0，如下图所示：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438989281-56b9ae8c-1802-4a4e-aca7-59e31cf52115.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/25755350/1742438990742-ac8a0bd9-8758-4b12-b291-b7c618c0fcde.png)
