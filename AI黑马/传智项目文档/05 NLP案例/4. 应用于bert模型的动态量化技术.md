### 学习目标[¶](http://121.199.45.168:8008/4/#_1)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2024/png/25755350/1730543597544-cbf637ce-960a-4a75-b6dd-c1c67e9df596.png)

---

### 相关知识[¶](http://121.199.45.168:8008/4/#_2)
+ 模型压缩:
...
### 数据集说明[¶](http://121.199.45.168:8008/4/#_3)
+ GLUE数据集合的介绍：
...
### 使用huggingface中的预训练BERT模型进行微调[¶](http://121.199.45.168:8008/4/#huggingfacebert)
+ 第一步: 安装必要的工具包并导入
+ 第二步: 下载数据集并使用脚本进行微调
+ 第三步: 设定全局配置并加载微调模型
+ 第四步: 编写用于模型使用的评估函数
...
#### 第一步: 安装核心的工具包并导入[¶](http://121.199.45.168:8008/4/#_4)
+ 安装核心工具包:
...
#### 第二步: 下载数据集并使用脚本进行微调[¶](http://121.199.45.168:8008/4/#_5)
+ 下载GLUE中的MRPC数据集:
...
#### 第三步: 设定全局配置并加载微调模型[¶](http://121.199.45.168:8008/4/#_6)
+ 设定全局配置:
...
### 使用动态量化技术对训练后的bert模型进行压缩[¶](http://121.199.45.168:8008/4/#bert)
+ 第一步: 将模型应用动态量化技术
+ 第二步: 对比压缩后模型的大小
...
#### 第一步: 将模型应用动态量化技术[¶](http://121.199.45.168:8008/4/#_7)
+ 应用动态量化技术:
...
#### 第二步: 对比压缩后模型的大小[¶](http://121.199.45.168:8008/4/#_8)
```plain
...
#### 第三步: 对比压缩后的模型的推理准确性和耗时[¶](http://121.199.45.168:8008/4/#_9)
```plain
...
#### 第四步: 序列化模型以便之后使用[¶](http://121.199.45.168:8008/4/#_10)
```plain
# 量化模型的保存路径
...
### 附录[¶](http://121.199.45.168:8008/4/#_11)
#### run_glue.py微调脚本代码[¶](http://121.199.45.168:8008/4/#run_gluepy)
...
> 来自: [应用于bert模型的动态量化技术 - NLP案例集](http://127.0.0.1:5500/%E5%BA%94%E7%94%A8%E4%BA%8Ebert%E6%A8%A1%E5%9E%8B%E7%9A%84%E5%8A%A8%E6%80%81%E9%87%8F%E5%8C%96%E6%8A%80%E6%9C%AF%20-%20NLP%E6%A1%88%E4%BE%8B%E9%9B%86.html)
>
