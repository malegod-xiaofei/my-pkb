...
if not docs or not docs[0].page_content.strip():
    raise ValueError("网页内容加载为空，无法创建 FAISS 向量库。请检查网络访问或网页结构是否变化。")

# DashScope 兼容接口要求 input 是字符串或字符串列表；关闭 LangChain 的 token-id 输入模式
...
if not documents:
    raise ValueError("文档切分结果为空，无法创建 FAISS 向量库。")
...
agent = create_agent(
    model=llm,
    tools=tools,
    system_prompt="你是一个中文问答助手。回答中华人民共和国民法典相关问题时，必须先使用 CivilCodeRetriever 工具检索资料，再根据检索结果回答。",
)
