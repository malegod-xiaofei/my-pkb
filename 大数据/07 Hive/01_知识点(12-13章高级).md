...
考虑上述因素，此处可将该参数设置为64G，如下：（总内存的1/2到2/3之间）
...
考虑上述因素，此处可将该参数设置为16。（一个核，分配4G内存）
...
load data local inpath '/opt/module/hive-3.1.3/datas/tiaoyou/order_detail.txt' overwrite into table order_detail partition(dt='2020-06-14');
...
load data local inpath '/opt/module/hive-3.1.3/datas/tiaoyou/payment_detail.txt' overwrite into table payment_detail partition (dt = '2020-06-14');
...
load data local inpath '/opt/module/hive-3.1.3/datas/tiaoyou/product_info.txt' overwrite into table product_info;
...
load data local inpath '/opt/module/hive-3.1.3/datas/tiaoyou/province_info.txt' overwrite into table province_info;

## 1.3 Explain 查看执行计划（重点）

### 1.3.1 Explain 执行计划概述
