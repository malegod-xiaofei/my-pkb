...
测试过程中，可以通过使用 sc.queueStream(queueOfRDDs)来创建 DStream，每一个推送到这个队列中的RDD，都会作为一个DStream 处理。

### 3.1.2 案例实操

- 需求：循环创建几个 RDD，将RDD 放入队列。通过 SparkStream 创建 Dstream，计算 WordCount
- 编写代码
...
Receiver[String](StorageLevel.MEMORY_ONLY) { //最初启动的时候，调用该方法，作用为：读数据并将数据发送给 Spark
...
import com.atguigu.handler.{

BlackListHandler, DateAreaCityAdCountHandler,
...
- 开窗确定时间范围；
- 在窗口内将数据转换数据结构为((adid,hm),count); 3）
